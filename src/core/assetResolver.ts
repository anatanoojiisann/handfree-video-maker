...
export async function resolveAssets(projectDir:string,storyboard:any,assetList:any,warnings:string[]){const assetsDir=path.join(projectDir,'assets');await fs.mkdir(assetsDir,{recursive:true});
 ...
 generatePlaceholder(out,scene.on_screen_text,storyboard.aspect_ratio);a.local_path=out;a.status=scene.preferred_asset_source==='playwright_screenshot'?'captured':'generated';warnings.push('Placeholder generated');}
 return assetList;}
